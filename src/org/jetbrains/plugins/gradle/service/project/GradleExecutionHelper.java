...
public class GradleExecutionHelper
{
			...
			@NotNull final OutputStream standardOutput, @NotNull final OutputStream standardError)
	{
		...
		if(!jvmArgs.isEmpty())
		{
			...
			Collection<String> merged = buildEnvironment != null ? mergeJvmArgs(buildEnvironment.getJava().getJvmArguments(), jvmArgs) : jvmArgs;
			...
			switch(settings.getDistributionType())
			{
				...
				case WRAPPED:
					if(settings.getWrapperPropertyFile() != null)
					{
						...
						{
							Distribution distribution = new DistributionFactoryExt(StartParameter.DEFAULT_GRADLE_USER_HOME).getWrappedDistribution
									(propertiesFile);
							try
							{
	...
	{
		InputStream stream = Init.class.getResourceAsStream("/org/jetbrains/plugins/gradle/tooling/internal/init/buildSrcInit.gradle_");
		try
		{
			if(stream == null)
			{
	...
	private static void replaceTestCommandOptionWithInitScript(@NotNull List<String> args)
	{
		...
		if(!testIncludePatterns.isEmpty())
		{
			...
			InputStream stream = Init.class.getResourceAsStream("/org/jetbrains/plugins/gradle/tooling/internal/init/testFilterInit.gradle_");
			try
			{
				if(stream == null)
				{
